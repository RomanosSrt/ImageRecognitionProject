...
# Paths
users_path = 'ModifiedDataset/users50-300.csv'
...
for file in os.listdir(reviews_folder):
    # Extract movie name
    movie = re.sub(r' \d{4}\.csv$', '', file).replace('_', ':')
    ...
    for _, row in ratings.iterrows():
        ...
        if rating != 'Null':  # Convert to float
...
matrixR_df.to_csv('ModifiedDataset/user_matrix.csv')
